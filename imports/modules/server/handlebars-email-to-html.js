...
export default (handlebarsMarkup, context, options) => {
  if (handlebarsMarkup && context) {
    ...
    /* The base template expects an object with a "content" key (required), "applicationName (required)" and
     * optional "twitterHandle" and "twitterUrl" keys. These values are automatically supplied to the base
     * template so that they don't also need to be provided to the skinny template which may or may not use them.
